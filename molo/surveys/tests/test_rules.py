...
from ..rules import SurveySubmissionDataRule, GroupMembershipRule, SurveyResponseRule
...
class TestSurveyResponseRule(TestCase, MoloTestCaseMixin):
    def setUp(self):
        ...
        SurveysIndexPage.objects.first().add_child(instance=self.survey)
        SurveysIndexPage.objects.first().add_child(instance=self.personalisable_survey)
        self.personalisable_survey.save_revision()
        self.survey.save_revision()
        PersonalisableSurveyFormField.objects.create(
            field_type='singleline', label='Singleline Text', page=self.personalisable_survey)
        MoloSurveyFormField.objects.create(
            field_type='singleline', label='Singleline Text', page=self.survey)

    def submit_survey(self, survey, user):
        submission = survey.get_submission_class()
        data = {field.clean_name: 'super random text' for field in survey.get_form_fields()}
        submission.objects.create(user=user, page=survey, form_data=json.dumps(data))
